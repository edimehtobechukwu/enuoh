...
export function Navbar() {
  ...
  const logoBg = navTheme === "dark" ? "bg-white" : "bg-black";
  const logoText = navTheme === "dark" ? "text-black" : "text-white";
  const linkColor = navTheme === "dark" ? "text-[#d1d5dc] hover:text-white" : "text-[#666] hover:text-black";
  const borderColor = navTheme === "dark" ? "border-white/10" : "border-black/5";
  const glassBg = navTheme === "dark" ? "bg-white/10" : "bg-white/70";
    ...
    <>
      ...
      >
        <div className="mx-auto max-w-[1444px] pointer-events-auto">
          <div className={`backdrop-blur-md border ${borderColor} rounded-[15px] h-[66px] px-6 flex items-center justify-between relative shadow-lg transition-colors duration-300 ${glassBg}`}>
            <div className={`absolute inset-0 border ${borderColor} rounded-[15px] pointer-events-none transition-colors duration-300`} />

            {/* Logo */}
            <Link to="/" className="flex items-center gap-[8px] z-50">
              <div className={`w-[32px] h-[32px] ${logoBg} rounded-full flex items-center justify-center transition-colors duration-300`}>
                <span className={`font-bold ${logoText} font-oswald text-[18px] leading-[28px] transition-colors duration-300`}>E</span>
              </div>
              <span className={`${textColor} font-bold font-oswald text-[20px] leading-[28px] tracking-[-0.5px] transition-colors duration-300`}>Enuoh</span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-[32px]">
              {links.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`${linkColor} text-[11px] uppercase tracking-[1.1px] font-bold font-oswald transition-colors duration-300`}
                >
                  {link.name}
                </Link>
              ))}
            ...
            {/* Mobile Menu Toggle */}
            <button
              className={`md:hidden ${textColor} z-50 w-10 h-10 flex items-center justify-center transition-colors duration-300`}
